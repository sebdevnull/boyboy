...
{
    if (addr == core::io::IoReg::Serial::SB) {
...
)
{
    ...
    for (long i = 0; i < rom_banks; i++) {
        std::fill(
            rom_data.begin() + i * core::cartridge::mbc::RomBankSize,
            rom_data.begin() + (i + 1) * core::cartridge::mbc::RomBankSize,
            std::byte(i)
        );
    }
