...
inline void expect_r(const boyboy::core::cpu::Cpu& cpu, const InstrParam& p)
{
    auto tgt = p.target();
    if (tgt.is_r8()) {
...
inline void expect_at_addr(const boyboy::core::cpu::Cpu& cpu, const InstrParam& p)
{
...
{
    EXPECT_EQ(cpu.get_register(boyboy::core::cpu::Reg16Name::HL), p.target_addr() + 1)
        << "HL increment mismatch: " << p.name;
}

inline void expect_hl_dec(const boyboy::core::cpu::Cpu& cpu, const InstrParam& p)
{
    EXPECT_EQ(cpu.get_register(boyboy::core::cpu::Reg16Name::HL), p.target_addr() - 1)
        << "HL increment mismatch: " << p.name;
}
...
inline void expect_pop(const boyboy::core::cpu::Cpu& cpu, const InstrParam& p)
{
...
inline void expect_call(const boyboy::core::cpu::Cpu& cpu, const InstrParam& p)
{
    uint16_t sp   = cpu.get_sp();
    uint16_t addr = boyboy::common::utils::to_u16(cpu.read_byte(sp + 1), cpu.read_byte(sp));
